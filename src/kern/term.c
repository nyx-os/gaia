...
/* Originally written by segfaultdev & me */
...
static inline void draw_rect_h(int x, int y, int w, int h, uint32_t color_1,
                               uint32_t color_2)
{
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
            uint32_t color_a = ((color_1 >> 0) & 0xFF) * ((w - 1) - j) +
                               ((color_2 >> 0) & 0xFF) * j;
            uint32_t color_b = ((color_1 >> 8) & 0xFF) * ((w - 1) - j) +
                               ((color_2 >> 8) & 0xFF) * j;
...
static inline void draw_rect_v(int x, int y, int w, int h, uint32_t color_1,
                               uint32_t color_2)
{
    for (int i = 0; i < h; i++) {
        for (int j = 0; j < w; j++) {
            uint32_t color_a = ((color_1 >> 0) & 0xFF) * ((h - 1) - i) +
                               ((color_2 >> 0) & 0xFF) * i;
            uint32_t color_b = ((color_1 >> 8) & 0xFF) * ((h - 1) - i) +
                               ((color_2 >> 8) & 0xFF) * i;
...
static inline void draw_circle(int x, int y, int radius, uint32_t color_1,
                               uint32_t color_2)
{
    for (int i = y - radius; i <= y + radius; i++) {
        for (int j = x - radius; j <= x + radius; j++) {
            ...
            uint32_t color_b = ((color_1 >> 8) & 0xFF) * ((radius - 1) - root) +
                               ((color_2 >> 8) & 0xFF) * root;
            uint32_t color_c =
                    ((color_1 >> 16) & 0xFF) * ((radius - 1) - root) +
                    ((color_2 >> 16) & 0xFF) * root;
            uint32_t color_d =
                    ((color_1 >> 24) & 0xFF) * ((radius - 1) - root) +
...
static void write_char(char c)
{
    ...
    if (do_escape_code(c)) {
        plot_char(c, term.cursor_x * ISO_CHAR_WIDTH + term.start_x,
                  term.cursor_y * ISO_CHAR_HEIGHT + term.start_y, term.fg);
        term.cursor_x++;
    }
}

#define BACKGROUND_COLOR 0x3b6ea5
//#define SHADOW_COLOR 0x008C8C
#define SHADOW_COLOR 0x315b88
...
void term_init(charon_t *charon)
{
    ...
    // Clear the screen
    draw_rect(0, 0, term.fb.width, term.fb.height, BACKGROUND_COLOR);

    // Shadow circles
    draw_circle(start_x - 1, start_y - 1, 16, SHADOW_COLOR, BACKGROUND_COLOR);
    draw_circle(start_x + width, start_y - 1, 16, SHADOW_COLOR,
                BACKGROUND_COLOR);
    draw_circle(start_x - 1, start_y + height, 16, SHADOW_COLOR,
                BACKGROUND_COLOR);
    draw_circle(start_x + width, start_y + height, 16, SHADOW_COLOR,
                BACKGROUND_COLOR);
    ...
    // Title bar
    draw_rect(start_x + 4, start_y + 4, width - 8, 2 + ISO_CHAR_HEIGHT,
              0x0000AA);

    // Title bar's text
    const char *s = "Gaia Console";
    int x = 0;

    while (*s) {
        plot_char(*s, x * ISO_CHAR_WIDTH + ((width / 2)), start_y + 5,
                  0xFFFFFF);
        x++, s++;
    }
    ...
    draw_rect(textbox_x + 1, textbox_y + (textbox_height - 2),
              textbox_width - 2, 1, 0xAAAAAA);
    draw_rect(textbox_x + (textbox_width - 2), textbox_y + 1, 1,
              textbox_height - 2, 0xAAAAAA);

    // Super cute shadow
    draw_rect_h(start_x + width, start_y, 16, height, SHADOW_COLOR,
                BACKGROUND_COLOR);
    draw_rect_h(start_x - 16, start_y, 16, height, BACKGROUND_COLOR,
                SHADOW_COLOR);
    draw_rect_v(start_x, start_y + height, width, 16, SHADOW_COLOR,
                BACKGROUND_COLOR);
    draw_rect_v(start_x, start_y - 16, width, 16, BACKGROUND_COLOR,
                SHADOW_COLOR);

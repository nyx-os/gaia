...
clang_tidy() {
  run-clang-tidy -use-color -quiet #-checks=cppcoreguidelines*,clang-analyzer-*
}

clang_format() {
  find src/ \( -name '*.hpp' -o -name '*.cpp' -o -name '*.h' -o -name '*.c' \) -exec "clang-format" -i '{}' \;
  echo -e "\033[1m- Formatted Code\033[0m"
}

add_licenses() {
  find src/ \( -name '*.hpp' -o -name '*.cpp' \) -exec "./scripts/license.py" '{}' \;
